...
func (r *TemplateActivityLineRepository) FindByTemplateActivityID(templateActivityID uuid.UUID) (*[]entity.TemplateActivityLine, error) {
	...
	if err := r.DB.Preload("TemplateQuestion.Questions.QuestionOptions").Preload("TemplateActivity").Preload("TemplateQuestion.Questions.AnswerType").Preload("TemplateQuestion.Questions.QuestionResponses").Where("template_activity_id = ?", templateActivityID).Find(&templateActivityLines).Error; err != nil {
		return nil, err
	}
