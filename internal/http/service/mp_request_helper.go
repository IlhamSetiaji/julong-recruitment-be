...
func MPRequestServiceFactory(log *logrus.Logger) IMPRequestService {
...
func (h *MPRequestService) CheckPortalData(req *response.MPRequestHeaderResponse) (*response.MPRequestHeaderResponse, error) {
	...
	if err != nil {
		h.Log.Errorf("[MPRequestService] error when send find employee by id message: %v", err)
		return nil, err
	}

	if requestorExist == nil {
		h.Log.Errorf("[MPRequestService] requestor with id %s is not exist", req.RequestorID.String())
	...
	if req.DepartmentHead != nil {
		...
		if deptHeadExist == nil {
			h.Log.Errorf("[MPRequestService] department head with id %s is not exist", req.DepartmentHead.String())
	...
	if err != nil {
		h.Log.Errorf("[MPRequestService] error when send find emp organization by id message: %v", err)
		return nil, err
	}
	...
	if err != nil {
		h.Log.Errorf("[MPRequestService] error when send find job level by id message: %v", err)
		return nil, err
	}

	if jobLevelExist == nil {
		h.Log.Errorf("[MPRequestService] job level with id %s is not exist", req.JobLevelID.String())
		return nil, errors.New("job level is not exist")
	}

	h.Log.Infof("[MPRequestService] check portal data success %v", requestorExist.EmployeeJob)
